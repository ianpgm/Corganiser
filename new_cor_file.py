...
if "generate" not in form:
	...
	for i in range(len(header_title_list)):
		...
		if header_variable_list[i] == "hole_name":
			print("<TD rowspan=7><b>Tips:</b><ul><li>Specify depths of \"beginning\" or \"end\" to indicate the top and bottom of the hole.<li>Make sure both request number and name fields are filled before adding a new request or sample<li>To delete a sample or sample series, just empty all of its fields then hit \"Update\"<li>Trying to delete more than one request at once will result in an error.</ul>")
...
if "generate" in form:
	filename = "/var/www/output_files/" + header_variable_dict['hole_name'] + "_" + header_variable_dict['starting_core'] + "_" + id_generator() + ".cor"
	...
	os.system("python corganiser_universal_3.py " + filename)
